...
export function GraphViewer() {
  ...
  const activeTasks = demoMode ? demoTasks : [];
      ...
      {!demoMode && (
        <div className="flex-1 border border-dashed border-border rounded-lg bg-card p-12 text-center flex flex-col items-center justify-center">
          ...
          <p className="text-sm text-muted-foreground mt-2 max-w-md">
            Run the demo workflow to preview agent tasks, Band handoffs, activity history, and analytics data.
          </p>

          {liveAgentCount > 0 && (
            <p className="text-sm text-muted-foreground mt-4">
              Current live graph nodes: {liveAgentCount}
            </p>
          )}
        </div>
      )}
