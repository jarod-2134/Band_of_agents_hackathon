...
async def list_org_members(
    ...
    query = text("""
        SELECT u.id, u.email, u.display_name, om.member_role as role, 
               'active' as status, NOW() as last_active
        FROM org_members om
...
async def change_member_role(
    ...
    target = await db.execute(text(
        "SELECT member_role FROM org_members WHERE org_id = :org_id AND user_id = :user_id;"
    ...
    ), {"role": payload.role, "org_id": org_context["org_id"], "user_id": id})
    await db.commit()
