...
from app.core.security import hash_password, verify_password, blind_compare, generate_tokens 
...
async def login_user(
    ...
    # Accessing fields via tuple indices / mappings mapping securely
    ...
    try:
        ...
        orgs = await db.execute(
            text("SELECT org_id, member_role, org_slug FROM org_members WHERE user_id = :user_id"), 
            {"user_id": user_dict["id"]}
        )
        org_list = orgs.fetchall()
        user_orgs = []

        for user_org in org_list:
            org_row = user_org._mapping
            user_orgs.append(
                {
                    "org_id": str(org_row["org_id"]),
                    "org_slug": org_row["org_slug"],
                    "role": org_row["member_role"]
                }
            )

        access_token, raw_refresh_token, refresh_token_hash = generate_tokens(user_dict["id"], email_clean, user_orgs)
...
async def refresh_token_rotation(
    ...
    token_dict = token_record._mapping
    if token_dict["used_at"] is not None:
        logger.warning(f"REUSE DETECTION ALERT: Token ID {token_dict['id']} reused! Purging user tokens.")
        await db.execute(text("DELETE FROM refresh_tokens WHERE user_id = :user_id;"), {"user_id": token_dict["user_id"]})
        await db.commit()
        response.delete_cookie(key="refresh_token")
        raise PlatformException(401, "compromised_session", "Session compromised. Re-authentication required.")

    now_utc = datetime.now(timezone.utc)
    expires_at_utc = token_dict["expires_at"].replace(tzinfo=timezone.utc) if token_dict["expires_at"].tzinfo is None else token_dict["expires_at"]
    ...
    try:
        await db.execute(
            ...
            {"now": now_utc, "id": token_dict["id"]}
        )

        user_email_query = text("SELECT email FROM users WHERE id = :user_id;")
        user_res = await db.execute(user_email_query, {"user_id": token_dict["user_id"]})
        ...
        access_token, raw_refresh, new_refresh_hash = generate_tokens(token_dict["user_id"], user_email, user_orgs)
...
async def register_user(payload: RegisterSchema, response: Response, db: AsyncSession = Depends(get_db)):
    ...
    try:
        # Step A: Provision the organization first
        ...
        # Step B: Provision the user profile
        ...
        # Step C: Map entry inside org_members junction table
        await db.execute(
            text("""
        ...
        # Step D: Handle active token credentials generation
        access_token, raw_refresh_token, refresh_token_hash = generate_tokens(user_id, email_clean, user_orgs)
...
# --- 5. OAUTH2 PASSWORD FORM TOKEN ENDPOINT ---
@router.post("/token")
async def login_for_token(
    ...
    if not user:
        blind_compare(form_data.password)
        raise PlatformException(401, "invalid_credentials", "Email or password incorrect.")
